    ...
    SqidsEncoder<int> idEncoder) : IIntakeMatchingService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return release is null
            ...
            : new IntakeReleaseMatch(
                ...
                release.Locale,
                release.RegionCode,
                ToImageUrl(release.FrontImageLocation));
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var evidenceGlobalDiscId = match.EvidenceSet.GlobalDiscId;
        var mismatch = submittedGlobalDiscId is not null && !match.EvidenceSet.ExactGlobalDiscIdMatch;
        ...
        var promotedSource = match.EvidenceSet.ScanLogs.FirstOrDefault()?.Source ?? IntakeSource.Unknown;
        if (!logExisted && !stagedLogExists)
        {
            (stagedLogExists, failureReason, promotedSource) = await CopyBestScanLogAsync(
                match.EvidenceSet.ScanLogs,
                stagedLogPath,
                cancellationToken);
        }
        ...
        if (promotion is null)
        {
            promotion = new IntakePromotion
            {
                ...
                EvidenceSetId = match.EvidenceSet.Id,
            };
        ...
        CancellationToken cancellationToken)
    {
        ...
        var candidate = discs
            ...
            {
                Disc = d,
                EvidenceSet = SelectEvidenceSet(d.Evidence, normalizedGlobalDiscId),
            })
            .Where(x => x.EvidenceSet is not null)
            .OrderByDescending(x => x.EvidenceSet!.ExactGlobalDiscIdMatch)
        ...
        return new LoadedDiscMatch(disc, releaseDisc, candidate.EvidenceSet!);
    }
    ...
    private static SelectedEvidenceSet? SelectEvidenceSet(
    ...
    private async Task<(bool Copied, string? FailureReason, IntakeSource Source)> CopyBestScanLogAsync(
        IReadOnlyList<IntakeDiscEvidence> scanLogs,
        string destinationPath,
        CancellationToken cancellationToken)
    {
        string? lastFailure = null;
        foreach (var evidence in scanLogs)
        {
            try
            {
                ...
                await assetStore.Save(
                    ...
                    ContentTypes.TextContentType,
                    cancellationToken);
                return (true, null, evidence.Source);
            }
            catch (Exception ex)
            {
        ...
        return (
            ...
            scanLogs.FirstOrDefault()?.Source ?? IntakeSource.Unknown);
    }
