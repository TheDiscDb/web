...
public partial class AddDiscsToBoxset : ComponentBase
{
    ...
    private async Task LoadContributions()
    {
        ...
        {
            // Need the boxset slug to filter; without it we'd pull every contribution.
            return;
        }
        ...
        try
        {
            // Server-side filter: only contributions whose ReleaseSlug matches the boxset slug.
            // Boxset members must share the boxset's slug as their release directory name; any
            // other contribution is unaddable, so don't show them.
            var filter = new UserContributionFilterInput
            {
                ReleaseSlug = new StringOperationFilterInput { Eq = boxsetSlug }
            };

            var result = await ContributionClient.GetEligibleContributions.ExecuteAsync(100, null, filter);
            if (result != null && result.IsSuccessResult())
            {
        ...
        try
        {
            ...
            if (result.Data?.AddDiscToBoxset?.Errors is { Count: > 0 } errors)
            {
                ...
                errorMessage = error switch
                {
                    ...
                    IAddDiscToBoxset_AddDiscToBoxset_Errors_MismatchedReleaseSlugError e => e.Message,
