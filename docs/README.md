...
| [Rapid Disc Intake](FEATURE-RAPID-DISC-INTAKE.md) | 🟡 In progress | Administrator-only ContributionBuddy workflow for rapidly staging release metadata, images, disc hashes/IDs, and MakeMKV logs without identifying titles. Matching staged evidence later prefills the normal owned web contribution flow; staged records are never published directly. |
| [Partial Contributions](FEATURE-PARTIAL-CONTRIBUTIONS.md) | 🔵 Proposed | Let users contribute part of a release/disc and mark what's missing so others can complete it: missing discs (combo/boxset/series), partially-identified discs, and empty/unidentified discs (incl. auto-flagged Engram/API submissions). Partial state is a contributor declaration (canned reason + free-text) persisted in `/data` (`partial { reason, note }` on `disc*.json` and `release.json`), surfaced via detail-page badges and a partial-data hub. Marker writes go DB-first then batch-sync to `/data` like edit suggestions. Missing discs resolve via a retargeted disc-contribution; partial discs via suggested edits. Phase 1 = declaration + persistence + auto-flag + badges + sync; Phase 2 = hub + resolution/clearing; Phase 3 = analytics/notifications. |
| [AACS Disc ID](FEATURE-AACS-DISC-ID.md) | 🔵 Proposed | Add the AACS Disc ID (`SHA1(AACS/Unit_Key_RO.inf)`, 40-hex) as a second, globally-stable disc hash alongside the existing content hash, then backfill it across every disc via user contributions. Verified feasible: needs no decryption keys (distinct from the VUK), BD/UHD share the rule (DVD deferred), and the file is already reachable through the existing `AddDisc` File System Access flow — so it computes **client-side in WASM** and only the hex string is sent. Backfill matches a user's disc to the DB via the current content-hash and attaches the Disc ID as an **add-only** change. Phase 0 = computation spike; Phase 1 = browser PoC; Phase 2 = capture + persist (models/DB/`disc*.json`/sync rail); Phase 3 = backfill campaign (match + add-only attach + progress + guardrails); Phase 4 = ecosystem (API lookup, keydb cross-ref, DVD, dedup key). |
